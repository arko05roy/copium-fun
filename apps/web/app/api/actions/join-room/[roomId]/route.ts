...
import { buildJoinRoomGet, buildJoinRoomPost } from "@/lib/join-room-action";
...
export async function GET(
  ...
  const payload = await buildJoinRoomGet(roomId, baseUrl(req));
  if ("error" in payload) {
...
export async function POST(
  ...
  const payload = await buildJoinRoomPost(roomId, account);
  if ("error" in payload) {
