...
export async function buildCopyActionGet(
  ...
  if (trade.pulse_status !== "open") return { error: `pulse ${trade.pulse_status ?? "unknown"}` };
  ...
  return {
    ...
    description: `${verb} ${side.toUpperCase()} — ${trade.pulse_question}`,
    label: `${verb} · ${(Number(stake) / 1_000_000).toFixed(2)} USDT`,
    links: {
      actions: [
        {
          type: "transaction",
          label: `${verb} on devnet`,
...
export async function buildCopyActionPost(
  ...
  const post = await createPostResponse({
    fields: {
      type: "transaction",
      transaction: tx,
      message: `${verb} ${trade.agent_name} · ${side.toUpperCase()} · ${trade.pulse_question.slice(0, 64)}`,
    },
  });
